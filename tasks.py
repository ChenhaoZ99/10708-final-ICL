...
# TODO: added here to normalize the scales
def squared_percentage_error(ys_pred, ys):
    return ((ys - ys_pred) / ys).square()
...
def get_task_sampler(
    ...
    # print("========= Structure initialized here =========")
...
class LinearRegression(Task):
    ...
    def get_metric():
        # TODO: changed here
        # return squared_percentage_error
...
class Relu2nnRegression(Task):
    def __init__(
        ...
        if pool_dict is None and seeds is None:
            self.W1 = distribution_fn(self.b_size, self.n_dims, hidden_layer_size)
            self.W2 = distribution_fn(self.b_size, hidden_layer_size, 1)
        elif seeds is not None:
            ...
            for i, seed in enumerate(seeds):
                generator.manual_seed(seed)
                self.W1[i] = distribution_fn(
                    self.n_dims, hidden_layer_size, generator=generator
                )
                self.W2[i] = distribution_fn(hidden_layer_size, 1, generator=generator)
        else:
            ...
            self.W1 = self.W1 * weight_scale + weight_bias
            self.W2 = self.W2 * weight_scale + weight_bias
