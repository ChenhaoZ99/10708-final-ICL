...
def basic_plot(metrics, models=None, trivial=1.0):
    ...
    for name, vs in metrics.items():
        # if not name[1] is None:
        #     name = f"N({name[0]}, {name[1]})"
        # else:
        #     name = f"hidden={name[0]}"
...
def collect_results(run_dir, df, valid_row=None, rename_eval=None, rename_model=None, save_path=None):
    all_metrics = {}
    for _, r in df.iterrows():
        ...
        metrics = get_run_metrics(run_path, skip_model_load=True, save_path=save_path)
